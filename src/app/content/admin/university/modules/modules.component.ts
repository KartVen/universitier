...
import { ModuleForPage, ModuleService } from '../../../../_services/module.service';
...
export class ModulesComponent implements AfterViewInit {
  protected dataSource: ModuleForPage[] = [];
  ...
  protected handleApi(
    ...
    this.moduleService.getModules(size, page - 1, 'id', 'ASC', filters).subscribe({
      next: res => {
  ...
  protected add() {
    this.router.navigate(['university', 'module-add']);
  }

  protected open(id: number): void {
    this.router.navigate(['university', 'modules', id]);
  }

  protected edit(id: number): void {
    this.router.navigate(['university', 'modules', id, 'edit']);
  }
}

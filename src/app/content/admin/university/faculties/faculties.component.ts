...
import { FacultyForPage, FacultyService } from '../../../../_services/faculty.service';
...
export class FacultiesComponent implements AfterViewInit {
  protected dataSource: FacultyForPage[] = [];
  ...
  protected handleApi(
    ...
    this.facultyService.getFaculties(size, page - 1, 'id', 'ASC', filters).subscribe({
      next: res => {
  ...
  protected add() {
    this.router.navigate(['university', 'faculty-add']);
  }

  protected open(id: number): void {
    this.router.navigate(['university', 'faculties', id]);
  }

  protected edit(id: number): void {
    this.router.navigate(['university', 'faculties', id, 'edit']);
  }
}
